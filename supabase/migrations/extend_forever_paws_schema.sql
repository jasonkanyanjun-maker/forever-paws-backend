-- Forever Paws Pet Memorial App - 扩展现有数据库架构
-- 基于现有的videos表，添加新的表结构
...
-- 扩展现有videos表，添加pet_id关联
ALTER TABLE public.videos 
ADD COLUMN IF NOT EXISTS pet_id UUID REFERENCES public.pets(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'completed' CHECK (status IN ('processing', 'completed', 'failed'));
...
CREATE INDEX IF NOT EXISTS idx_videos_pet_id ON public.videos(pet_id);
CREATE INDEX IF NOT EXISTS idx_letters_user_id ON public.letters(user_id);
CREATE INDEX IF NOT EXISTS idx_letters_pet_id ON public.letters(pet_id);
CREATE INDEX IF NOT EXISTS idx_letters_sent_at ON public.letters(sent_at DESC);
